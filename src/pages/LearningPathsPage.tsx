...
export const LearningPathsPage: React.FC = () => {
  ...
  const modules = DRIVE_FOLDERS.map(folder => {
    const hasAccess = isAdmin || profile?.folderAccess?.[folder.id] === true || 
      profile?.videoAccess?.[folder.id] === true;
    const isWatched = videoProgress.includes(folder.id);
    ...
    return {
      ...
      estimatedTime: 75 // Average 1h 15m DevOps video lecture
    };
  });
