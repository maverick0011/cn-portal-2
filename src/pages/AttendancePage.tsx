      ...
      snapshot.docs.forEach((meetingDoc) => {
        ...
        if (!attendanceUnsubscribes[meetingId]) {
          attendanceUnsubscribes[meetingId] = onSnapshot(
            ...
            (error) => {
              handleFirestoreError(error, OperationType.LIST, `meetings/${meetingId}/attendance`);
            }
          );
        }
      });
            ...
            >
              ...
              <div className="p-6 flex-1 bg-white">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {(attendance[meeting.id] || []).filter(att => activeUsers.has(att.studentId)).length > 0 ? (
                    (attendance[meeting.id] || []).filter(att => activeUsers.has(att.studentId)).map((att: any, idx) => (
