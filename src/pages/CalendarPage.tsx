...
import { Calendar as CalendarIcon, Plus, Users, Clock, Trash2, CheckCircle2, ChevronLeft, ChevronRight, X, Sparkles } from 'lucide-react';
...
export const CalendarPage: React.FC = () => {
  ...
  const [newGoal, setNewGoal] = useState({ title: '', isGlobal: false });
  ...
  const handleAddGoal = async (e: React.FormEvent) => {
    ...
    if (newGoal.isGlobal) {
      await createNotification('New Global Milestone', `A new global goal has been set: "${newGoal.title}"`, 'general');
    }
    setNewGoal({ title: '', isGlobal: false });
  ...
  const handleAddMeeting = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createNotification('New Session Scheduled', `A new session "${newMeeting.title}" has been added to the calendar.`, 'meeting');
        ...
        <div className="flex gap-2">
          {isAdmin && (
            <button
              ...
              className="flex items-center gap-2 bg-emerald-500 text-white px-6 py-3 rounded-2xl font-bold text-sm hover:bg-emerald-600 shadow-lg shadow-emerald-500/20 active:scale-95 transition-all"
            >
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="grid grid-cols-7 gap-px bg-slate-100">
            {Array.from({ length: startDayOfWeek }).map((_, idx) => (
              <div key={`empty-${idx}`} className="min-h-[100px] bg-slate-50/50 cursor-default" />
            ))}
            {days.map((day, idx) => {
        ...
        <div className="space-y-4">
                  ...
                  <div className="relative z-10">
                    ...
                    {meeting.meetingLink && (
                       <a 
                         ...
                         className="mt-3 block w-full text-center py-2 bg-primary text-white rounded-xl text-[10px] font-bold hover:shadow-lg transition-all"
                       >
                         Join Meeting
                       </a>
                    )}
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
             ...
             <div className="space-y-3">
                {goals.filter(g => g.isGlobal || g.userId === user?.uid || isAdmin).map(goal => (
                  <div key={goal.id} className={cn(
                    "p-4 rounded-2xl border flex items-center justify-between",
                    goal.isGlobal ? "bg-emerald-50 border-emerald-100" : "bg-blue-50 border-blue-100"
                  )}>
                     <div>
                        <div className="flex items-center gap-2 mb-1">
                           <span className={cn(
                             "text-[8px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full",
                             goal.isGlobal ? "bg-emerald-200 text-emerald-700" : "bg-blue-200 text-blue-700"
                           )}>
                             {goal.isGlobal ? 'Global' : 'Personal'}
                           </span>
                           {isAdmin && !goal.isGlobal && (
                             <span className="text-[8px] font-bold text-slate-400">By {goal.userName}</span>
                           )}
                        </div>
                        <p className="text-xs font-bold text-slate-900">{goal.title}</p>
                     </div>
                     {(isAdmin || goal.userId === user?.uid) && (
                       <button 
                ...
                {goals.length === 0 && (
                   <p className="text-center p-4 text-xs text-slate-400 italic">No goals set yet.</p>
                )}
             </div>
          </div>
        </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
               ...
               <form onSubmit={handleAddGoal} className="space-y-4">
                  <div>
                     <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2 block">Goal Title</label>
                     <input 
                       required
                       value={newGoal.title}
                       ...
                       placeholder="e.g. Complete Docker Module"
                     />
                  </div>
                  {isAdmin && (
                    <div className="flex items-center gap-2">
                       <input 
                         type="checkbox"
                         checked={newGoal.isGlobal}
                         onChange={e => setNewGoal({...newGoal, isGlobal: e.target.checked})}
                         id="isGlobal"
                       />
                       <label htmlFor="isGlobal" className="text-xs font-bold text-slate-600">Make this a Global Goal for all students</label>
                    </div>
                  )}
                  <div className="flex justify-end gap-3 pt-4 border-t border-slate-50">
                     <button type="button" onClick={() => setShowGoalForm(false)} className="px-4 py-2 text-sm font-bold text-slate-400">Cancel</button>
                     <button type="submit" className="bg-emerald-500 text-white px-6 py-2 rounded-xl text-sm font-bold shadow-lg shadow-emerald-500/20">Set Goal</button>
                  </div>
               </form>
            </motion.div>
          </div>
              ...
              <form onSubmit={handleAddMeeting} className="p-6 overflow-y-auto space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2 block">Meeting Link</label>
                      <input 
