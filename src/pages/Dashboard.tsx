...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubLeader = onSnapshot(qLeader, (snap) => {
      // Filter out admins and limit to top 5
    ...
    if (user) {
      ...
      unsubProgress = onSnapshot(qProgress, (snap) => {
        const progress_local = snap.docs.map(d => d.data().videoId);
        const stats = DRIVE_FOLDERS.map(folder => progress_local.includes(folder.id));
