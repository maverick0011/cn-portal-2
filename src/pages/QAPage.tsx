...
export const QAPage: React.FC = () => {
  ...
  const handleStartEditQ = (qId: string, content: string) => {
    setEditingQId(qId);
    setEditingContent(content);
  };

  const handleSaveEditQ = async (qId: string) => {
    if (!editingContent.trim()) return;
    try {
      await updateDoc(doc(db, 'qa_questions', qId), {
        content: editingContent.trim()
      });
      setEditingQId(null);
      setEditingContent('');
    } catch (error: any) {
      handleFirestoreError(error, OperationType.UPDATE, 'qa_questions');
    }
  };
  ...
  const handleDeleteQ = async () => {
    if (!deleteQId) return;
    try {
      await deleteDoc(doc(db, 'qa_questions', deleteQId));
      setDeleteQId(null);
    } catch (error: any) {
      handleFirestoreError(error, OperationType.DELETE, 'qa_questions');
    }
  };
          ...
          >
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="flex items-center gap-2">
                ...
                {(isAdmin || (user && user.uid === q.userId)) && editingQId !== q.id && (
                  <button
                    onClick={() => handleStartEditQ(q.id, q.content)}
                ...
                {(isAdmin || (user && user.uid === q.userId)) && (
                  <button
            ...
            {editingQId === q.id ? (
              <div className="flex flex-col gap-3 p-4 bg-slate-50 dark:bg-slate-850 rounded-2xl border border-slate-100 dark:border-slate-800 mb-6">
                <textarea
                  value={editingContent}
                  onChange={(e) => setEditingContent(e.target.value)}
                  className="w-full bg-white dark:bg-slate-800 text-slate-800 dark:text-white rounded-xl p-3.5 text-sm focus:outline-none focus:ring-1 focus:ring-primary border border-slate-200 dark:border-slate-700 resize-none font-medium h-24"
                />
                <div className="flex gap-2 justify-end">
                  <button
                    onClick={handleCancelEditQ}
                    className="px-4 py-2 bg-slate-200 hover:bg-slate-300 dark:bg-slate-750 dark:hover:bg-slate-700 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={() => handleSaveEditQ(q.id)}
                    className="px-4 py-2 bg-primary hover:bg-blue-700 text-white rounded-xl text-xs font-bold shadow-sm shadow-primary/20 transition-colors"
                  >
                    Save Changes
                  </button>
                </div>
