...
export const MessagesPage: React.FC = () => {
  ...
  const handleDeleteMessageConfirm = async () => {
    ...
    const collName = activeTab === 'announcements' ? 'announcements' : 'community_chat';
    try {
      await deleteDoc(doc(db, collName, deleteMsgId));
      setDeleteMsgId(null);
    } catch (error: any) {
  ...
  const handleSaveEdit = async (msgId: string) => {
    ...
    const collName = activeTab === 'announcements' ? 'announcements' : 'community_chat';
    try {
      await updateDoc(doc(db, collName, msgId), {
        text: editingText.trim()
      });
      setEditingMsgId(null);
      setEditingText('');
    } catch (error: any) {
      handleFirestoreError(error, OperationType.UPDATE, collName);
    }
  };
            ...
            return (
              <React.Fragment key={msg.id}>
                ...
                >
                  ...
                  <div className={cn(
                    "relative group flex items-center gap-2 max-w-full",
                    ...
                    {editingMsgId === msg.id ? (
                      <div className="flex flex-col gap-2 min-w-[240px] max-w-md p-3 bg-slate-100 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-750">
                        <textarea
                          value={editingText}
                          onChange={(e) => setEditingText(e.target.value)}
                          className="w-full bg-white dark:bg-slate-700 text-slate-800 dark:text-white rounded-xl p-2.5 text-xs focus:outline-none focus:ring-1 focus:ring-primary border border-slate-200 dark:border-slate-600 resize-none font-medium h-16"
                        />
                        <div className="flex gap-1.5 justify-end">
                          <button
                            onClick={handleCancelEdit}
                            className="px-2.5 py-1 bg-slate-200 hover:bg-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 rounded-lg text-[10px] font-bold text-slate-700 dark:text-slate-300 transition-colors"
                          >
                            Cancel
                          </button>
                          <button
                            onClick={() => handleSaveEdit(msg.id)}
                            className="px-2.5 py-1 bg-primary hover:bg-blue-700 text-white rounded-lg text-[10px] font-bold shadow-sm shadow-primary/20 transition-colors"
                          >
                            Save
                          </button>
                        </div>
